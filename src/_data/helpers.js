...
module.exports = {
  ...
  getComponent(collection, name, type) {
    ...
    if (!component) {
      throw new Error(
        `Component: ${name} not found in collection for type: ${type}`
      );
    }
    return component;
  },
  filterComponents(components, collection) {
    ...
    return components.map((componentData) => {
      ...
      const componentType =
        component.data.type || (component.data.tags && component.data.tags[0]);
      if (!type) {
        throw new Error(
          `Type not found for: ${name} make collection or add type`
        );
      }
