...
const viewsIcon = () =>
  html`<svg width="36" height="27" viewBox="0 0 36 27">
    <defs>
      <style>
        .a {
          fill: none;
...
const durationIcon = () =>
  html`<svg width="27" height="27" viewBox="0 0 27 27">
    <defs>
      <style>
        .a {
          fill: none;
    ...
    <g transform="translate(-1.5 -1.5)">
      ...
      />
      <path class="a" d="M18,9v9l6,3" transform="translate(-3.75 -2)" />
    </g>
  </svg>`;
...
const renderStats = (stats, video, renderOpen = false) => html`<div
  class="stats"
>
  ${stats
    ? html`<span aria-label="${stats.duration.str}"
          ><span aria-hidden="true">${stats.duration.time}</span
          >${durationIcon()}</span
        >
        <p aria-label="${shortenLargeNumber(stats.views)} views" class="views">
          <span aria-hidden="true">${shortenLargeNumber(stats.views)}</span
          >${viewsIcon()}
        </p>`
    : ""}
...
const renderInfo = (title, channel, channelId, viewMore) => html`<p
    title="${title}"
    class="video-title"
  >
    ${title}
  ...
  ${channelId
    ? html`<a
        ...
        >${channel}</a
      >`
    : html`<p class="video-channel">${channel}</p>`}`;
...
const _playlistItem = (video, _onTap, index, viewMore) => {
  ...
  return html`<button
    aria-hidden="${viewMore ? false : true}"
    tabindex="${viewMore ? "0" : "-1"}"
    aria-label="Watch video ${videoTitle}, duration: ${stats
      ? stats.duration.str
      : ""}"
    data-index="${index}"
    @click="${_onTap}"
    ...
    <figure>
      <div class="thumbnail">
        <img
          decoding="async"
          loading="lazy"
          width="${video.smallThumbnail.width}"
          height="${video.smallThumbnail.height}"
          alt="${videoTitle}"
          src="${thumbnail}"
        />
      </div>
  ...
  _onTap(e) {
    ...
    if (iframe) {
      iframe.contentWindow.postMessage(
        window.JSON.stringify({ event: "command", func: "stopVideo" }),
        "*"
      );
    }
    this._setCurrentVideo(
    ...
    return html`
      <div class="container">
        ...
        <div class="video-size video-item">
          <div @click="${
            this._onPlay
          }" class="video-wrapper video-aspect" style="${thumbnail}">
            ${_litYoutubeHTML(this.currentVideo)}
          </div>
          <div class="current-video-info">
          ${renderInfo(
            videoObj.title,
            videoObj.channelTitle,
            videoObj.channelId
          )}
